...
export class PostRepo implements IPostRepo {
  ...
  /**
   * NEW FEATURE: Get Unpopular Posts (least points) ADDED TO US009
   * @param offset
   * @returns 15 posts ordered by least points
   */
  ...
  /**
   * Function to check if a member has posts
   * @param memberId memberId
   * @returns boolean
   */
  public async hasPostsByMemberId(memberId: MemberId | string): Promise<boolean> {
    memberId = memberId instanceof MemberId ? (<MemberId>memberId).id.toString() : memberId;
    const PostModel = this.models.Post;
    const detailsQuery = this.createBaseQuery();
    detailsQuery.where['member_id'] = memberId;
    const postsCount = await PostModel.count(detailsQuery);
    return postsCount > 0;
  }
}
